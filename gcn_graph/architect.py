...
class Architect(object):
    ...
    def step(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer, unrolled, mask=False):
        self.optimizer.zero_grad()
        if unrolled:
            self._backward_step_unrolled(input_train, target_train, input_valid, target_valid, eta, network_optimizer)
        else:
            self._backward_step(input_valid, target_valid, mask=mask)
        for weights in self.model.arch_parameters():
            if not weights.requires_grad:
                weights.grad = None
        self.optimizer.step()

    def _backward_step(self, input_valid, target_valid, mask=False):
        if mask:
