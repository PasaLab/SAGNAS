...
class Architect(object):
    ...
    def _compute_unrolled_model(self, data, eta, network_optimizer):
        loss = self.model._loss(data, is_valid=False)
    ...
    def step(self, data, eta, network_optimizer, unrolled, mask=False):
        
        self.optimizer.zero_grad()
        if unrolled:
            self._backward_step_unrolled(data, eta, network_optimizer)
        else:
            self._backward_step(data, mask=mask)
        for weights in self.model.arch_parameters():
            if not weights.requires_grad:
                weights.grad = None
        self.optimizer.step()

    def _backward_step(self, data, mask=False):
        # if mask:
        #     logits = self.model(input_valid)[input_valid.valid_mask]
        #     loss = self.model._criterion(logits, torch.squeeze(target_valid))
        # else:
        #     loss = self.model._loss(input_valid, target_valid)
        loss = self.model._loss(data, is_valid=True)
        loss.backward()
    ...
    def _backward_step_unrolled(self,data, eta, network_optimizer):
        ...
        unrolled_loss = unrolled_model._loss(data, is_valid=True)
